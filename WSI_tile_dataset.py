...
class WSI_tile_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        path = random.choice(path_lt)
        image = Image.open(path)
        feat_0 = self.transform(image)
        feat_1 = self.transform(image)
        return (feat_0, feat_1), label
