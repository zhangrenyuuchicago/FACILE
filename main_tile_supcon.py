...
from networks.models import SupConResNet
...
def parse_option():
    ...
    # method
    parser.add_argument('--method', type=str, default='SimCLRTile',
                        choices=['SupConTile', 'SimCLRTile'], help='choose method')
...
def set_loader(opt):
    ...
    transform = augmentation.get_aug(name='simclr', image_size=224, train=True, train_classifier=None)
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if opt.method == 'SupConTile':
            loss = criterion(features, labels)
        elif opt.method == 'SimCLRTile':
