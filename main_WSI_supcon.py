...
from networks.models import SupConResNetWSI
...
def parse_option():
    ...
    parser.add_argument('--method', type=str, default='SupConWSI',
                        choices=['SupConWSI', 'SimCLRWSI'], 
                        help='choose method')
...
def set_loader(opt):
    transform = augmentation.get_aug(name='simclr', image_size=224, 
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if opt.method == 'SupConWSI':
            loss = criterion(features, labels)
        elif opt.method == 'SimCLRWSI':
